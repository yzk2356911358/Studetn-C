...
int readf()
{
	...
	while (!feof(pf)) {
		...
		fscanf(pf, "%d%s%s%d%s%s%s%s", &p->num, p->name, p->sex, &p->age,
			p->birth, p->phone, p->classs, p->adrs);
		//插入信息
		if (ph == NULL) {
			ph = p;
			tail = p;
...
{
	if (p) {
		printf("学号:%d\t姓名:%s\t性别:%s\t年龄:%d\t出生年月:%s\t电话:%s\t班级:%s\t地址:%s\t\n",
			p->num, p->name, p->sex, p->age, p->birth, p->phone, p->classs, p->adrs);
	}
	else {
		printf("所给信息无效");
	}
...
int savef()
{
	...
	while (p) {
		fprintf(pf, " %d\t%s\t%s\t%d\t%s\t%s\t%s\t%s\t", p->num, p->name,
			p->sex, p->age, p->birth, p->phone, p->classs, p->adrs);
		p = p->next;
	}
	if (!pf) {
		fclose(pf);
